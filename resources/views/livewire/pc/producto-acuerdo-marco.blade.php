<div class="p-6 bg-white dark:bg-zinc-900 min-h-screen">
    ...
    <div class="mb-6 bg-zinc-50 dark:bg-zinc-800 rounded-xl p-6 shadow-sm border border-zinc-200 dark:border-zinc-700">
        <div class="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-6">
            <div>
                <flux:heading size="lg" class="text-zinc-900 dark:text-white">Administrador de Productos Acuerdo Marco</flux:heading>
                <flux:text class="mt-2 text-zinc-600 dark:text-zinc-400">Gestiona los productos registrados en los acuerdos marco del sistema.</flux:text>
            </div>
    ...
    <div class="mb-6 bg-zinc-50 dark:bg-zinc-800 rounded-xl p-6 shadow-sm border border-zinc-200 dark:border-zinc-700">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
                ...
                <flux:input type="search" wire:model.live="search" placeholder="Buscar por código, orden, RUC, proveedor, entidad, descripción..." icon="magnifying-glass" class="w-full mt-1" />
            </div>
            <div>
                <flux:label class="text-sm font-medium text-zinc-700 dark:text-zinc-300">Código Acuerdo Marco</flux:label>
            ...
            <div class="flex items-center justify-between">
                ...
                <div class="flex items-center gap-4">
                    <div class="flex items-center gap-2">
                        <flux:button wire:click="toggleGroupByOrdenElectronica" variant="{{ $groupByOrdenElectronica ? 'primary' : 'outline' }}" size="sm" icon="view-columns">
                    ...
                    <span class="text-sm text-zinc-500 dark:text-zinc-400">{{ $productos ? $productos->count() : 0 }} productos encontrados</span>
                </div>
            </div>
        </div>
                    ...
                    <tr>
                        <th class="px-6 py-4 text-left text-xs font-semibold text-zinc-600 dark:text-zinc-300 uppercase tracking-wider cursor-pointer hover:text-blue-600 transition-colors"
                            ...
                            <div class="flex items-center space-x-2">
                                ...
                                <flux:icon name="{{ $sortField === 'cod_acuerdo_marco' ? ($sortDirection === 'asc' ? 'arrow-up' : 'arrow-down') : 'arrows-up-down' }}" class="w-4 h-4" />
                            </div>
                        </th>
                        <th class="px-6 py-4 text-left text-xs font-semibold text-zinc-600 dark:text-zinc-300 uppercase tracking-wider cursor-pointer hover:text-blue-600 transition-colors"
                            ...
                            <div class="flex items-center space-x-2">
                                <span>Proveedor</span>
                                <flux:icon name="{{ $sortField === 'razon_proveedor' ? ($sortDirection === 'asc' ? 'arrow-up' : 'arrow-down') : 'arrows-up-down' }}" class="w-4 h-4" />
                            </div>
                        </th>
                        <th class="px-6 py-4 text-left text-xs font-semibold text-zinc-600 dark:text-zinc-300 uppercase tracking-wider cursor-pointer hover:text-blue-600 transition-colors"
                            ...
                            <div class="flex items-center space-x-2">
                                <span>Entidad</span>
                                <flux:icon name="{{ $sortField === 'razon_entidad' ? ($sortDirection === 'asc' ? 'arrow-up' : 'arrow-down') : 'arrows-up-down' }}" class="w-4 h-4" />
                            </div>
                        </th>
                        <th class="px-6 py-4 text-left text-xs font-semibold text-zinc-600 dark:text-zinc-300 uppercase tracking-wider cursor-pointer hover:text-blue-600 transition-colors"
                            ...
                            <div class="flex items-center space-x-2">
                                ...
                                <flux:icon name="{{ $sortField === 'descripcion_ficha_producto' ? ($sortDirection === 'asc' ? 'arrow-up' : 'arrow-down') : 'arrows-up-down' }}" class="w-4 h-4" />
                            </div>
                        </th>
                        <th class="px-6 py-4 text-left text-xs font-semibold text-zinc-600 dark:text-zinc-300 uppercase tracking-wider cursor-pointer hover:text-blue-600 transition-colors"
                            ...
                            <div class="flex items-center space-x-2">
                                <span>Marca</span>
                                <flux:icon name="{{ $sortField === 'marca_ficha_producto' ? ($sortDirection === 'asc' ? 'arrow-up' : 'arrow-down') : 'arrows-up-down' }}" class="w-4 h-4" />
                            </div>
                        </th>
                        <th class="px-6 py-4 text-left text-xs font-semibold text-zinc-600 dark:text-zinc-300 uppercase tracking-wider cursor-pointer hover:text-blue-600 transition-colors"
                            ...
                            <div class="flex items-center space-x-2">
                                ...
                                <flux:icon name="{{ $sortField === 'precio_unitario' ? ($sortDirection === 'asc' ? 'arrow-up' : 'arrow-down') : 'arrows-up-down' }}" class="w-4 h-4" />
                            </div>
                        </th>
                            ...
                            <tr class="bg-blue-50 dark:bg-blue-900/20">
                                <td colspan="7" class="px-6 py-3">
                                    <div class="flex items-center gap-2">
                                        ...
                                        <span class="font-semibold text-blue-800 dark:text-blue-200">Orden Electrónica: {{ $ordenElectronica }}</span>
                                        <span class="text-sm text-blue-600 dark:text-blue-300">({{ $productosGrupo ? $productosGrupo->count() : 0 }} productos)</span>
                                    </div>
                                </td>
                            </tr>
                            <!-- Productos del grupo -->
                            @foreach($productosGrupo as $producto)
                                <tr wire:key="producto-{{ $producto->id }}" class="hover:bg-zinc-50 dark:hover:bg-zinc-700/50 transition-colors">
                                    ...
                                    <td class="px-6 py-4 text-sm text-zinc-900 dark:text-zinc-300">
                                        <div class="max-w-xs truncate" title="{{ $producto->descripcion_ficha_producto }}">
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-zinc-900 dark:text-zinc-300">
                                        <span class="font-medium">S/ {{ number_format($producto->precio_unitario, 2) }}</span>
                                    </td>
                                    <td class="px-6 py-4 text-sm">
                                        <div class="flex items-center gap-2">
                                            <flux:button wire:click="verDetalleProducto({{ $producto->id }})" size="xs" variant="primary" icon="eye" title="Ver detalle" class="hover:bg-blue-600 transition-colors">Ver Detalle</flux:button>
                                        </div>
                                    </td>
                                </tr>
                            @endforeach
        ...
        <div class="space-y-6">
            <div class="border-b pb-4 mb-2 flex items-center gap-3">
                ...
                <div>
                    ...
                    <flux:text class="mt-1 text-zinc-500">Información completa del producto de acuerdo marco.</flux:text>
                </div>
            </div>
            ...
            <div class="space-y-6">
                ...
                <div class="bg-zinc-50 dark:bg-zinc-800 rounded-lg p-4 shadow-sm border">
                    <div class="flex items-center gap-2 mb-4">
                        ...
                        <flux:heading size="md">Todos los Productos de la Orden: {{ $producto->orden_electronica }}</flux:heading>
                        <span class="text-sm text-zinc-500">({{ $productosOrden && $productosOrden->count() > 0 ? $productosOrden->count() : 0 }} productos)</span>
                    </div>
                    <div class="overflow-x-auto">
                        <table class="w-full text-sm">
                            <thead class="bg-zinc-100 dark:bg-zinc-700">
                                <tr>
                                    <th class="px-3 py-2 text-left font-medium text-zinc-700 dark:text-zinc-300">Descripción</th>
                                    <th class="px-3 py-2 text-left font-medium text-zinc-700 dark:text-zinc-300">Marca</th>
                                    <th class="px-3 py-2 text-left font-medium text-zinc-700 dark:text-zinc-300">Cantidad</th>
                                    <th class="px-3 py-2 text-left font-medium text-zinc-700 dark:text-zinc-300">Precio Unit.</th>
                                    <th class="px-3 py-2 text-left font-medium text-zinc-700 dark:text-zinc-300">Total</th>
                                </tr>
                            </thead>
                            <tbody class="divide-y divide-zinc-200 dark:divide-zinc-700">
                                @if($productosOrden && $productosOrden->count() > 0)
                                    @foreach($productosOrden as $productoOrden)
                                    <tr class="hover:bg-zinc-50 dark:hover:bg-zinc-700/50 {{ $productoOrden->id === $producto->id ? 'bg-blue-50 dark:bg-blue-900/20' : '' }}">
                                        <td class="px-3 py-2 text-zinc-900 dark:text-zinc-300">
                                            <div class="max-w-xs truncate" title="{{ $productoOrden->descripcion_ficha_producto }}">
                                ...
                                @else
                                    <tr>
                                        <td colspan="5" class="px-3 py-4 text-center text-zinc-500 dark:text-zinc-400">
                ...
                <div class="bg-zinc-50 dark:bg-zinc-800 rounded-lg p-4 shadow-sm border">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                        <div>
                            ...
                            <flux:text class="text-lg font-bold text-green-600">S/ {{ number_format($producto->precio_unitario, 2) }}</flux:text>
                        </div>
                        <div>
                        ...
                        <div>
                            ...
                            <flux:text class="text-lg font-bold text-blue-600">S/ {{ number_format($producto->total_monto, 2) }}</flux:text>
                        </div>
                        <div>
                        ...
                        <div>
                            ...
                            <flux:text class="text-sm">S/ {{ number_format($producto->igv_entrega, 2) }}</flux:text>
                        </div>
                        <div>
                            <flux:label class="text-xs font-medium text-zinc-600">Flete</flux:label>
                            <flux:text class="text-sm">S/ {{ number_format($producto->monto_flete, 2) }}</flux:text>
                        </div>
                    </div>
                </div>
            </div>
