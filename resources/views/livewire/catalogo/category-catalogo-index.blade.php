        ...
        <div class="bg-gradient-to-r from-blue-500 to-blue-600 rounded-lg p-4 text-white">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-2xl font-bold">{{ $categorias->total() }}</p>
                </div>
                <flux:icon name="folder" class="w-8 h-8 opacity-80" />
            </div>
        </div>
        <div class="bg-gradient-to-r from-green-500 to-green-600 rounded-lg p-4 text-white">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-2xl font-bold">{{ $categorias->where('isActive', 1)->count() }}</p>
                </div>
        ...
        <div class="bg-gradient-to-r from-red-500 to-red-600 rounded-lg p-4 text-white">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-2xl font-bold">{{ $categorias->where('isActive', 0)->count() }}</p>
                </div>
                ...
                <tbody class="divide-y divide-zinc-200 dark:divide-zinc-700">
                    @forelse ($categorias as $categoria)
                        <tr wire:key="categoria-{{ $categoria->id }}"
                            ...
                            <td class="px-6 py-4 text-sm">
                                @if ($categoria->logo)
                                    <div class="flex items-center gap-2">
                                        <img src="{{ asset('storage/' . $categoria->logo) }}"
                                            alt="{{ $categoria->name }}"
                            ...
                            <td class="px-6 py-4 text-sm">
                                @if ($categoria->archivo)
                                    <div class="flex items-center justify-center">
                                        <a href="{{ asset('storage/' . $categoria->archivo) }}"
                                            download="{{ $categoria->name }}_archivo.{{ pathinfo($categoria->archivo, PATHINFO_EXTENSION) }}"
                            ...
                            <td class="px-6 py-4 text-sm">
                                <span
                                    class="px-2 py-1 inline-flex text-xs leading-5 font-semibold rounded-full {{ $categoria->isActive ? 'bg-green-100 text-green-800 dark:bg-green-900 dark:text-green-200' : 'bg-red-100 text-red-800 dark:bg-red-900 dark:text-red-200' }}">
                                    {{ $categoria->isActive ? 'Activa' : 'Inactiva' }}
                                </span>
                            </td>
                            <td class="px-6 py-4 text-sm">
                                <div class="flex items-center gap-2">
                                    <flux:button wire:click="editarCategoria({{ $categoria->id }})" size="xs"
                                        variant="primary" icon="pencil" title="Editar categoría"
                                        class="hover:bg-blue-600 transition-colors"></flux:button>
                                    <flux:button wire:click="eliminarCategoria({{ $categoria->id }})" size="xs"
        ...
        <form wire:submit.prevent="guardarCategoria">
            <div class="space-y-6">
                <div class="border-b pb-4 mb-2 flex items-center gap-3">
                    ...
                    <div>
                        <flux:heading size="lg">{{ $categoria_id ? 'Editar Categoría' : 'Nueva Categoría' }}
                        </flux:heading>
                        ...
                        <div class="mt-1 space-y-3">
                            ...
                            @if ($fondoPreview)
                                <div class="relative inline-block group">
                                    ...
                                    <div class="mt-2 text-center">
                                        <flux:text class="text-xs text-green-600 font-medium">Vista previa del fondo
                                        </flux:text>
                                    </div>
                                </div>
                            @endif
                            ...
                            @if ($categoria_id && !$fondoPreview)
                                <div
                                    class="flex items-center gap-3 p-4 bg-gradient-to-r from-green-50 to-green-100 dark:from-green-900/20 dark:to-green-800/20 rounded-xl border border-green-200 dark:border-green-800">
                                    <div class="flex-shrink-0">
                                    ...
                                    <div class="flex-1">
                                        ...
                                        <p class="text-sm text-green-700 dark:text-green-300">El fondo actual se
                ...
                <div class="bg-zinc-50 dark:bg-zinc-800 rounded-lg p-4 shadow-sm border">
                    ...
                    <div>
                        <flux:label>Archivo de la categoría</flux:label>
                        <div class="mt-1 space-y-3">
            ...
            <div class="flex justify-end gap-2 mt-8 border-t pt-4 bg-white dark:bg-zinc-900 sticky bottom-0 z-10">
                ...
                <flux:button type="submit" variant="primary">{{ $categoria_id ? 'Actualizar' : 'Crear' }} Categoría
                </flux:button>
            </div>
        </form>
    </flux:modal>
    <!-- Modal Confirmar Eliminación -->
    <flux:modal wire:model="modal_form_eliminar_categoria">
        <div class="space-y-6">
            <div>
                <flux:icon name="exclamation-triangle" class="w-12 h-12 text-red-500 mx-auto mb-4" />
                <flux:heading size="lg" class="text-center">¿Eliminar categoría?</flux:heading>
                <flux:text class="text-center text-zinc-600 dark:text-zinc-400">
                    Esta acción no se puede deshacer. Se eliminarán todos los datos asociados a esta categoría.
                </flux:text>
            </div>
            <div class="flex justify-end gap-2">
                <flux:button wire:click="$set('modal_form_eliminar_categoria', false)">Cancelar</flux:button>
                <flux:button wire:click="confirmarEliminarCategoria" variant="danger">Eliminar Categoría</flux:button>
            </div>
        </div>
    </flux:modal>
</div>
